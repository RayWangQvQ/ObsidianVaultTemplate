...
modificationDate: 2024-10-27 11:35
---
...
- 如果想更换主页头图，可以把图片文件放到`99.Attachments/Files`文件夹里，然后主页属性的`banner`里引用
- 主页整体由一个叫“contribution widget”的插件构成（现版本已改名为“components”，但本库目前没有更新到这个版本）参考视频可以点[这里](https://www.bilibili.com/video/BV1TJ4m1x76K)，这是一个还在内测阶段的插件，所以没有办法在插件市场找到

## 原则

该模板遵循以下原则：
- 自下而上
	- 要允许笔记的关系、标签和分类自然的生长出来，而不是试图一开始就自上而下地设计好
	- 如果做一件事不是因为有意志力，而是因为不需要意志力，那么离成功就不远了
- 应使用Tag、双链和MOC取代物理文件夹
	- 使用Tag和双链维护笔记间的关系
	- 使用MOC维护笔记的聚合和分类
	- 应尽量弱化使用物理文件夹分类，所有笔记，都应优先考虑放到`02.Notes`文件夹下，且不需要物理文件夹层级，除非你有充分理由需要放到其他地方
- Less is more
	- 内容大于样式，舒适大于美观
	- 不要主动使用html，只使用markdown进行书写
	- 如无必要，勿增实体，尤其是花里胡哨的东西
- 每篇笔记尽量原子化，职责单一，聚合交给MOC
